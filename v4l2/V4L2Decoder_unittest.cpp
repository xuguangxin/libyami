...
void getResolution(int fd, uint32_t& width, uint32_t& height, uint32_t& dpbSize)
{
    struct v4l2_format format;
    memset(&format, 0, sizeof(format));
    format.type = V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE;
    ASSERT_IOCTL(VIDIOC_G_FMT, &format);
...
TEST(V4L2DecoderTest, APITest) {
    ...
    ASSERT_EQ(V4L2_CAP_VIDEO_CAPTURE_MPLANE | V4L2_CAP_VIDEO_OUTPUT_MPLANE | V4L2_CAP_STREAMING
        , caps.capabilities);
    ...
    streamOnOutput(fd);
    waitForFormatChange(fd);

    uint32_t width, height, dpbSize;
    getResolution(fd, width, height, dpbSize);
