...
static FrameData g_h264data[] = {
    ...
    { g_avc8x18, N_ELEMENTS(g_avc8x18), { 8, 18 } },
...
void checkOutput(SharedPtr<IVideoDecoder>& decoder, VideoFormatInfo& format, uint32_t& outFrames)
{
...
TEST_P(DecodeApiTest, Format_Change)
{
    ...
    while (frames.getFrame(buffer, info)) {
        ...
        //check forrmat
        EXPECT_EQ(YAMI_DECODE_FORMAT_CHANGE, decoder->decode(&buffer));
        const VideoFormatInfo* format = decoder->getFormatInfo();
        ASSERT_TRUE(format);
        EXPECT_EQ(info.width, format->width);
        EXPECT_EQ(info.height, format->height);
        allocator->onFormatChange(format);

        //get output previous output. which will meet previous format
        checkOutput(decoder, prevFormat, outFrames);
